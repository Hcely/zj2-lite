...
public interface CacheHelper {
    ...
    default <T> T get(final Class<T> type, final String key, Function<String, T> getter, long timeout, boolean ignoreErr) {
        if(StringUtils.isEmpty(key)) { return null; }
        return getCache(CacheUtil.getFullKey(type, key), key, getter, timeout, ignoreErr);
    }

    default <T> T get(final String prefix, final String key, Function<String, T> getter, long timeout, boolean ignoreErr) {
        if(StringUtils.isEmpty(key)) { return null; }
    ...
    <T> T getCache(final String cacheKey, final String dataKey, Function<String, T> getter, long timeout, boolean ignoreErr);
}